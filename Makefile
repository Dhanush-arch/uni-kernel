...
kernel_elf := unikernel.$(ARCH).elf
stripped_kernel_elf := unikernel.$(ARCH).stripped.elf
...
TESTCARGOFLAGS += --package unikernel_kernel -Z unstable-options
...
build:
	$(MAKE) build-crate
	cp target/$(ARCH)/$(build_mode)/unikernel_kernel $(kernel_elf)
...
.PHONY: iso
iso: build
	$(PROGRESS) MKISO unikernel.iso
	mkdir -p isofiles/boot/grub
	cp boot/grub.cfg isofiles/boot/grub/grub.cfg
	cp $(stripped_kernel_elf) isofiles/unikernel.elf
	grub-mkrescue -o unikernel.iso isofiles
