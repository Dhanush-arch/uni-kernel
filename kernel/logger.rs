use alloc::boxed::Box;
use unikernel_runtime::print::{get_debug_printer, set_debug_printer, Printer};
use unikernel_utils::ring_buffer::RingBuffer;
...
macro_rules! debug_warn {
    ($fmt:expr) => {
        if cfg!(debug_assertions) {
            ::unikernel_runtime::println!(concat!("\x1b[1;33mWARN: ", $fmt, "\x1b[0m"));
        }
    };
    ($fmt:expr, $($arg:tt)*) => {
        if cfg!(debug_assertions) {
            ::unikernel_runtime::println!(concat!("\x1b[1;33mWARN: ", $fmt, "\x1b[0m"), $($arg)*);
        }
    };
}
